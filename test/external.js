...
describe('External tests of unfisk, run from admin', () => {
  ...
  it('Should move the ASN when put into staging to same key in asns', async function () {
    this.timeout(5000);
    const staging_location = await con
      .put({
...
async function cleanup() {
  ...
  await con
    .get({ path: `/bookmarks/trellisfw/asn-staging/${asnkey}` })
    .then(async () =>
      con.delete({ path: `/bookmarks/trellisfw/asn-staging/${asnkey}` })
    ) // delete it
    .catch((e) => {}); // do nothing, didn't exist

  // delete the link in asn-staging if it exists
  await con
    .get({ path: `/bookmarks/trellisfw/asns/${asnkey}` })
    .then(async () =>
      con.delete({ path: `/bookmarks/trellisfw/asns/${asnkey}` })
    ) // delete it
    .catch((e) => {}); // do nothing, didn't exist
